...
class TreelistColumn(namedtuple("TreelistColumnBase", "id text editable")):
...
class ToolkitBase:
    ...
    grammar = [
        ...
        (
            "option",
            r"\((?P<checked> |x)\)\s+%s$" % _re_maybe_id_text,
            '"( ) text" or "(x) text"',
        ),
        (
            "checkbox",
        ...
        (
            "slider",
            r"\[\s*(?P<id>[a-zA-Z0-9_]+)\s*\:\s*(?P<min>\d+)\s*\-\+\-\s*(?P<max>\d+)\s*\]",
            "[id: 0 -+- 100]",
        ),
        ...
        (
            "treelist",
            r"\[\s*=%s(?:\((?P<columns>.*?)\))?\s*\]" % _re_maybe_id_text,
            '"[= Text]" or [= Text (column1, column2, ..)]',
        ),
        (
            "combo",
            r"\[%s_\s*(?:\((?P<values>.*?)\))?\s+v\s*\]" % _re_maybe_id_text,
            '"[Text_ v]" or "[Text_ (val1, val2, ...) v]',
        ),
        (
            "dropdown",
            r"\[%s(?:\((?P<values>.*?)\))?\s+v\s*\]" % _re_maybe_id_text,
            '"[Text v]" or "[Text (val1, val2, ...) v]',
        ),
    ...
    def parse_menu(
        self, parent, menudef, handlers, translations=None, translation_prefix=""
    ):
            ...
            for name, regex, _ in self.menu_grammar:
                ...
                if m:
                    ...
                    if name == "sub":
                        ...
                        self.menu_command(
                            parent, handler=getattr(handlers, d["id"]), **d
                        )
                    else:
    ...
    def treelist(
        self, parent, id=None, text="", columns=None, first_column_editable=False
    ):
...
class ListBinding:
    ...
    def _set_list(self, val):
        ...
        if not isinstance(val, list_model.ObsList):
            ...
            if self._list is not None:
                val.children_source(
                    self._list._children_source, self._list._has_children_source
                )
        l = self._list = val
        if l is not None:
            l.on_insert += self.on_insert
    ...
    def sort(self, key=None, ascending: bool = None, restore=False):
        ...
        if isinstance(key, str):
            ...
            keyfunc = key
            info = {}
        self._list.sort(keyfunc, reverse=not ascending, info=info)
